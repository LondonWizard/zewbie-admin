...
export default function RetailerPayouts() {
  ...
  useEffect(() => {
    document.title = 'Admin - Retailer Payouts'
  }, [])
  ...
  async function fetchPayouts() {
    try {
      ...
      const res = await api.get('/admin/payouts/retailers', { params })
      const data = res.data
      setPayouts(data?.data ?? data?.payouts ?? [])
      setTotalPages(data?.meta?.totalPages ?? data?.totalPages ?? 1)
    } catch {
      setError('Failed to load retailer payouts')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Retailer Payouts</h1>
        <p className="text-sm text-gray-500 mt-1">Track and manage scheduled and completed payouts to retailers</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <form onSubmit={handleSearch} className="flex-1 min-w-[240px]">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by retailer name or reference…"
              value={search}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    ...
                    <p className="text-gray-500">No retailer payouts found</p>
                  </td>
                </tr>
              ) : (
                payouts.map((p) => (
                  <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <Link to={`/retailers/${p.retailerId}`} className="font-medium text-indigo-600 hover:text-indigo-800">
                        {p.retailerName}
                      </Link>
                    </td>
